...
define( 'LIGHTWEIGHT_GA_VERSION', '1.4.3' );
...
class lightweight_ga_plugin_options {
    ...
    public function __construct() {
        ...
        if( !isset(get_option('lightweight_ga_settings')['tracking_id']) || empty(get_option('lightweight_ga_settings')['tracking_id']) ) {
    ...
    public function setup_lightweight_ga_message() {
        $settings_link = '<a href="' . esc_url(menu_page_url('lightweight-google-analytics', false)) . '" title="' . esc_attr__('Lightweight Google Analytics Settings', 'lightweight_ga') . '">';
        $message = __('Thank you for installing <strong>Lightweight Google Analytics</strong> - Remember to head to the %ssettings%s to finish setting up Google Analytics.', 'lightweight_ga');
        printf('<div class="lightweightga notice notice-success"><p>%s</p></div>', sprintf($message, $settings_link, '</a>'));
    }
